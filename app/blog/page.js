  ...
  return (
    <div className="py-20 bg-slate-50 dark:bg-slate-900 min-h-screen transition-colors">
      <div className="container mx-auto px-4 max-w-5xl">
        <h1 className="text-4xl font-bold text-slate-800 dark:text-white mb-12 text-center">Blog Du Lịch</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {posts.map((post) => (
              <Link href={`/blog/${post.slug}`} key={post._id} className="group">
                <div className="bg-white dark:bg-slate-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow h-full flex flex-col border border-transparent dark:border-slate-700">
                  ...
                  <div className="p-8 flex-1 flex flex-col">
                    ...
                    <h3 className="text-2xl font-bold mb-4 text-slate-800 dark:text-white group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    <p className="text-slate-600 dark:text-slate-300 mb-6 line-clamp-3 flex-1">{post.excerpt}</p>
                    <span className="text-orange-600 dark:text-orange-400 font-medium group-hover:translate-x-2 transition-transform inline-block">
                      Đọc tiếp &rarr;
                    </span>
                  </div>
                </div>
